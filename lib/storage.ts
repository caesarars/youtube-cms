...
const STORAGE_KEY = 'ancient_lens_shorts';

const SAMPLE_DATA: Short[] = [
  {
    id: 'sample-1',
...
export function saveShort(short: Short): void {
  const shorts = getShorts();
  const index = shorts.findIndex((s) => s.id === short.id);
  if (index >= 0) {
    shorts[index] = { ...short, updatedAt: new Date().toISOString() };
  } else {
    shorts.push(short);
  }
...
export function deleteShort(id: string): void {
  const shorts = getShorts().filter((s) => s.id !== id);
  localStorage.setItem(STORAGE_KEY, JSON.stringify(shorts));
}
